...
from orchestrator.chassis.profile import CARAVEL, chassis_top, locked_boundary_ports, resolve_profile
...
def test_profile_from_task_yaml_and_env(tmp_path, monkeypatch):
    ...
    assert locked_boundary_ports(tmp_path) == ("io_in", "io_out", "io_oeb")   # union of built-ins


def test_wrapper_block_is_matched_by_declared_name_only():
    m = lambda name, ports: SimpleNamespace(name=name, ports=[SimpleNamespace(name=p) for p in ports])
    ...
    assert detect_wrapper_block(mods2) == "blk"                    # module name matches the chassis top
