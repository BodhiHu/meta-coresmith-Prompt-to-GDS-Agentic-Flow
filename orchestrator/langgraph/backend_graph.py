...
def _recorded_integration_top(root: Path) -> tuple[str, str]:
    ...
    if not mod:
        return "", ""      # WP-49: a record without a module name is not a top
    return top, mod
...
async def init_design_node(state: BackendState) -> dict:
    ...
    if _rec_top:
        ...
        # WP-49: the backend never guesses a top from the files on disk; the
        # integration check records the candidate (top module + file).
        log("  [BACKEND] no recorded integration top "
            "(.coresmith/integration_result.json) -- the backend does not "
            "guess; run the integration check first", RED)
