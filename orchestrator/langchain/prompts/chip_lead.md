...
- `uarch_integration_review`: `approve` is the default — the reviewer edits
  ...
  the blocks and the contract. A wrapper the POLICY authored as a block and
  the engine adopted as the top (WP-24) is that block's own RTL, judged like
  any other block. At `uarch_integration_review` it is a stale
