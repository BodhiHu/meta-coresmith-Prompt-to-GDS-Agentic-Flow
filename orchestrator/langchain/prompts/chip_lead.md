...
- `uarch_integration_review`: `approve` is the default — the reviewer edits
  ...
  THE ENGINE ASSEMBLES THE CHIP TOP (Caravel tasks, default
  `CORESMITH_DETERMINISTIC_CARAVEL_TOP=1`): the block named
  `user_project_wrapper` is the PAD ADAPTER -- a LEAF that exposes the locked
  `io_in`/`io_out`/`io_oeb` pads plus its inward contract ports and nothing
  else. It must NOT instantiate the other blocks or route the contract edges;
  the integration check assembles `rtl/integration/user_project_wrapper.v`
  from the blocks and the contract and the conformance gate REJECTS any block
  that instantiates a sibling. Never `revise` to demand that the wrapper block
  "instantiate all children", "route the edges" or "become the structural
  top" (observed: two identical revise rounds on ax25_9600 attempt 2 -- the
  generator obeyed the gate, the review demanded the opposite). A wrapper
  block whose pads and inward ports match the contract is correct.
  OUTER WRAPPER BLOCKS (`openframe_project_wrapper` and the like) are not
  part of the submission: the graded top is the task's declared boundary
  (`user_project_wrapper` for Caravel tasks). The engine does NOT remove
  such a block for you (WP-35). If the diagram contains one, ask ONCE, with
  a concrete instruction naming the block, for its removal (`revise`); if
  it is already absent, approve. Never loop on it (observed: four identical
  revise rounds on ax25_9600).
  DERIVED INTEGRATION ARTIFACTS ARE NOT EVIDENCE AT REVIEW TIME: the files
  the ENGINE assembled under `rtl/integration/` (a `user_project_wrapper.v`
  whose integration record says `caravel_wrapper_assembled`, its `_pads.v`,
  the engine's chip top) are REGENERATED by the next integration check from
